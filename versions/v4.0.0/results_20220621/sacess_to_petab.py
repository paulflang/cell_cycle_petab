...
version = 'v4.0.0'
date = '20220621'
sacess_fn = os.path.join(base_dir, 'PEtab_PLang_problem_v25.xlsx')
parameter_table_fn = os.path.join(base_dir, version, f'parameters_{version}.tsv')
...
for i in range(len(parameter_df)):
    ...
    if par_id in res.columns:
        ...
        if parameter_df.loc[i, 'estimate'] != 1:
            raise Error(f'Parameter {par_id} should not have been estimated.')
